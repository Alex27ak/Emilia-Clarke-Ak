    ...
    try:
        ...
        if not is_exist:
            ...
            await add_stream_details(
                file.file_id, STREAM_LOG, log_file.id, file_unique_id
            )
        else:
            message_id = is_exist["message_id"]
            file_unique_id = is_exist["file_unique_id"]

        hash1 = file_unique_id[:6]
        #print(f"{STREAM_URL}watch/{str(message_id)}/video.mp4?hash={hash1}")
        return f"{STREAM_URL}watch/{str(message_id)}/video.mp4?hash={hash1}"
    except Exception as e:
        print(e)
        return STREAM_URL
